...
int vtkvmtkPolyDataScissors::IsEdgeInCell(vtkPolyData *input, vtkIdType edgePointId0, vtkIdType edgePointId1, vtkIdType cellId)
{
	...
	for (i=0; i<npts; i++)
	{
		if( (pointIdList->GetId(i) == edgePointId0 && pointIdList->GetId((i+1)%npts) == edgePointId1)
		   || (pointIdList->GetId(i) == edgePointId1 && pointIdList->GetId((i+1)%npts) == edgePointId0) )
		{
			return 1;
		}
	}
...
//	input->GetCellPoints(referenceCellId,npts,pts);
	...
	for (i=0; i<npts; i++)
	{
		if (pointIdList->GetId(i)!=linePointId1 && pointIdList->GetId(i)!=linePointId0)
		{
			edge[1] = pointIdList->GetId(i);
			break;
		}
	}
...
//				 input->GetCellPoints(cells[i],npts,pts);
				for (j=0; j<npts; j++)
				{
					if (pointIdList->GetId(j) != edge[0] && pointIdList->GetId(j) != edge[1])
					{
						edge[1] = pointIdList->GetId(j);
						break;
					}
				}
...
//			input->GetCellPoints(cellId,npts,pts);
			pointIdList = vtkSmartPointer<vtkIdList>::New();
			input->GetCellPoints( cellId, pointIdList );
			
			
			editedCellIds->InsertUniqueId(cellId);
			editedCells->InsertNextCell( pointIdList->GetNumberOfIds() );
			for (j=0; j<pointIdList->GetNumberOfIds(); j++)
			{
				duplicatePointLocation = duplicatedPointIds->IsId( pointIdList->GetId(j) );
				...
				{
					editedCells->InsertCellPoint( pointIdList->GetId(j) );
				}
			}
		}
	}
	...
	{
		input->GetPolys()->GetNextCell(pointIdList);
		
		if (editedCellIds->IsId(i)==-1)
		{
			newPolys->InsertNextCell(pointIdList);
		}
	}
	...
	{
		editedCells->GetNextCell(pointIdList);
		newPolys->InsertNextCell(pointIdList);
	}
