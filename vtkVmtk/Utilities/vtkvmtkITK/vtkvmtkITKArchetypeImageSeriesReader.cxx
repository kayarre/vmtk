...
int vtkvmtkITKArchetypeImageSeriesReader::RequestInformation(
  ...
  vtkInformationVector *outputVector)
{
  ...
  if (fileNameCollapsed.find("slicer:") != std::string::npos &&
    ...
    if (!itksys::SystemTools::FileExists (fileNameCollapsed.c_str()))
      {
      vtkErrorMacro( "vtkvmtkITKArchetypeImageSeriesReader::ExecuteInformation: Archetype file " << fileNameCollapsed.c_str() << " does not exist.");
      return 0;
      }
    }
  ...
  try
    ...
    if (this->FileNames.size() == 1)
      ...
      if (imageIO.GetPointer() == NULL)
        {
          vtkErrorMacro( "vtkvmtkITKArchetypeImageSeriesReader::ExecuteInformation: ImageIO for file " << fileNameCollapsed.c_str() << " does not exist.");
          return 0;
        }
      }
    else
    ...
    vtkErrorMacro( "vtkvmtkITKArchetypeImageSeriesReader::ExecuteInformation: Cannot open " << fileNameCollapsed.c_str() << ". "
  ...
  if (this->UseNativeScalarType)
    ...
    if (this->FileNames.size() == 1)
      ...
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::UCHAR)
        {
        scalarType = VTK_UNSIGNED_CHAR;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::CHAR)
        {
        scalarType = VTK_CHAR;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::USHORT)
        {
        scalarType = VTK_UNSIGNED_SHORT;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::SHORT)
        {
        scalarType = VTK_SHORT;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::UINT)
        {
        scalarType = VTK_UNSIGNED_INT;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::INT)
        {
        scalarType = VTK_INT;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::ULONG)
        {
        scalarType = VTK_UNSIGNED_LONG;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::LONG)
        {
        scalarType = VTK_LONG;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::FLOAT)
        {
        scalarType = VTK_FLOAT;
        }
      else if (imageIO->GetComponentType() == itk::CommonEnums::IOComponent::DOUBLE)
        {
      ...
      for( unsigned int f = 0; f < this->FileNames.size(); f++ )
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::UCHAR )
          {
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::CHAR )
          {
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::USHORT )
          {
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::SHORT )
          {
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::UINT )
          {
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::INT )
          {
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::ULONG )
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::LONG )
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::FLOAT )
          {
        ...
        if ( imageIO->GetComponentType() == itk::CommonEnums::IOComponent::DOUBLE )
          {
...
//----------------------------------------------------------------------------
void vtkvmtkITKArchetypeImageSeriesReader::AssembleNthVolume ( int n )
{
  this->FileNames.resize( 0 );
  // int nFiles = this->AllFileNames.size(); UNUSED
...
//----------------------------------------------------------------------------
void vtkvmtkITKArchetypeImageSeriesReader::GroupFiles ( int idxSeriesInstanceUID,
                                                    int idxContentTime,
                                                    int idxTriggerTime,
                                                    int idxEchoNumbers,
                                                    int idxDiffusionGradientOrientation,
...
//----------------------------------------------------------------------------
void vtkvmtkITKArchetypeImageSeriesReader::AnalyzeDicomHeaders()
{
...
//----------------------------------------------------------------------------
void vtkvmtkITKArchetypeImageSeriesReader::ParseDictionary()
{
...
//----------------------------------------------------------------------------
unsigned int vtkvmtkITKArchetypeImageSeriesReader::GetNumberOfItemsInDictionary()
{
  return this->Tags.size();
}

//----------------------------------------------------------------------------
bool vtkvmtkITKArchetypeImageSeriesReader::HasKey( char* tag )
{
  return this->Dictionary.HasKey( tag );
}

//----------------------------------------------------------------------------
const char* vtkvmtkITKArchetypeImageSeriesReader::GetNthKey( unsigned int n )
{
  if (n >= this->Tags.size())
  {
    return NULL;
...
//----------------------------------------------------------------------------
const char* vtkvmtkITKArchetypeImageSeriesReader::GetNthValue( unsigned int n )
{
...
//----------------------------------------------------------------------------
const char* vtkvmtkITKArchetypeImageSeriesReader::GetTagValue( char* tag )
{
...
//----------------------------------------------------------------------------
const char* vtkvmtkITKArchetypeImageSeriesReader::GetFileName( unsigned int n )
{
...
//----------------------------------------------------------------------------
unsigned int vtkvmtkITKArchetypeImageSeriesReader::AddFileName( const char* filename )
{
